...
function Ralio(hostname, username, password, version, project, team) {
  this.hostname = hostname;
  this.username = username;
  this.password = password;
  /**
   * the original package worked with a combination of 1.36 and 1.42 interfaces
   * we default to 1.42 if the user doesn't enter anything. This might break 
   * in the near future as rally moves beyond 2.0
   */
  this.version = typeof version !== 'undefined' ? version : "1.43";
  this.project = project;
  this.team = team;
  /**
   * needed for testability - the editor module exports a raw function 
   * and not an object so there's no direct way to stub it out.  :/
   */
  this._editor = editor; 
...
Ralio.prototype.rallyWsapiUrl = function (type, query, options) {
  ...
  return url.format({
    ...
    port: rally_url.port,
    pathname: options.pathname ? options.pathname : '/slm/webservice/' + this.version + '/' + type + '.js',
    query: query
  });
}
...
Ralio.prototype.bulk = function (query, callback) {
  // comments to help with 2.0 update
  //console.log('Ralio.prototype.bulk: query=' +  JSON.stringify(query));
  // query is:
  //        {"OBJECT-TYPE":{"fetch":"FIELD-NAMES",
  //                        "query":"QUERY-STRING"}}
...
Ralio.prototype.addComment = function (itemID, text, callback) {
  var self = this,
      options = {
        url: self.bulkUrl({"pathname":"/slm/webservice/" + self.version + "/conversationpost/create.js"}),
        method: 'POST',
    ...
    self.bulk({
      ...
      if (error !== null) {
        ...
        if (value.length) {
          ...
          var options = {
            url: self.bulkUrl({"pathname":"/slm/webservice/" + self.version + "/timeentryvalue/create.js"}),
            method: 'POST',
            json: {"TimeEntryValue":{
              "Project": project,
              "TimeEntryItem": item,
          ...
          if (error !== null) {
            ...
            if (item.length) {
              ...
              var itemCreation = setInterval(function() {
                ...
                var options = {
                  url: self.bulkUrl({"pathname":"/slm/webservice/" + self.version + "/timeentryitem/create.js"}),
                  method: 'POST',
                  json: {"TimeEntryItem":{
                    "Project": project,
                    "WorkProduct": story,
...
Ralio.prototype.headers =  function (headers) {
  return _.defaults(headers || {}, {
    ...
    "X-RallyIntegrationName": package_json.name,
    "X-RallyIntegrationVendor": package_json.repository.url,
    "X-RallyIntegrationVersion": package_json.version,
    "User-Agent": "Node.JS (request:" + package_json.dependencies.request + ")"
  });
}
...
Ralio.prototype.backlog = function (options, callback) {
  ...
  var query = {
    ...
    order: 'Rank',
    query: (options.tag ?
      '(((((Project.Name = "' + options.projectName + '") AND (Iteration = NULL)) AND (Release = NULL)) AND (DirectChildrenCount = 0)) AND (Tags.Name = "' + options.tag + '"))' :
      '(((( Project.Name = "' + options.projectName + '") AND (Iteration = NULL)) AND (Release = NULL)) AND (DirectChildrenCount = 0))'),
...
Ralio.prototype.story = function (storyID, callback) {
  ...
  if (type === 'US') {
    ...
    callback("Ralio's can only show detailed information of a defect(DE), story(US) or task(TA).");
  }
      ...
      if (error !== null) {
        ...
        stories = _.filter(stories, function (story) {
          return _.any(story.Tasks, function (task) {
            return task.Owner !== null && task.State === 'In-Progress';
          });
        });

        defects = _.filter(defects, function (defect) {
            return defect.Owner !== null && (defect.State === 'Open' || defect.State === "Reopened" || defect.ScheduleState === "In-Progress");
        });
...
Ralio.prototype.me = function (projectName, callback) {
//https://demo-west.rallydev.com/slm/webservice/1.43/user?query=%28Username%20=%202015@hackathon.com%29
  var rally_url = url.parse(this.hostname);
  var req = this.rallyWsapiRequest({url: "https://" + this.username + ":" + this.password + "@" + 
        rally_url.hostname + "/slm/webservice/" + this.version + "/user?query=(Username = \""  + this.username + "\")"});
  var options = {
    url: "https://" + this.username + ":" + this.password + "@" + 
        rally_url.hostname + "/slm/webservice/" + this.version + "/user?query=(Username = \""  + this.username + "\")",
    method: 'GET',
    json: {},
    strictSSL: false,
  ...
  this.request(options, function (err, results){
      if (err !== null) {
        callback(err, null);
      } else {
        var parseString = require('xml2js').parseString;
        parseString(results, function (err, result) {
          this.UserRef =  result.QueryResult.Results[0].Object[0].$.refObjectName;
        });
      }
  });
  var self = this, d = this.date();
  this.bulk(
    ...
    function (error, data) {
      if (error !== null) {
        ...
        stories = _.filter(stories, function (story) {
          return _.any(story.Tasks, function (task) {
            return task.Owner !== null && (task.State === 'In-Progress' || task.State === 'Defined') && task.Owner._refObjectName == this.UserRef;
          });
        });

        defects = _.filter(defects, function (defect) {
            return defect.Owner !== null && (defect.State !== 'Closed' && defect.State !== 'Fixed') && defect.Owner._refObjectName == this.UserRef;
        });
  ...
  var taskCreation = setInterval(function() {
    if (tagsObject.length === tags.length || Ralio.test) {
      self.story(storyID, function(error, data) {
        ...
        var options = {
          url: self.bulkUrl({"pathname":"/slm/webservice/" + self.version + "/task/create.js"}),
          method: 'POST',
          json: new_task,
          strictSSL: !Ralio.test
        };
...
Ralio.prototype.deleteTask = function (projectName, storyID, bulkurl, callback) {
  var self = this;
  ...
  self.getTask(storyID, function(error, task) {
    ...
    var bulk_result = self.bulkUrl({"pathname": deletePath});
    if (bulkurl != null) {
      bulk_result = bulkurl + deletePath;
    }
    var options = {
          url: bulk_result ,
          method: 'DELETE',
...
Ralio.prototype.task = function (option, projectName, storyID, taskName, tags, callback) {
  switch(option){
    ...
    case "delete": this.deleteTask(projectName, storyID, null , callback); break;
