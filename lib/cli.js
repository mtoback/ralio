...
function CLI(hostname) {
  ...
  this.configure();
  this.ralio = new Ralio(this.config.hostname, this.config.username, this.config.password, this.config.version,
    this.config.project, this.config.team);
}

CLI.prototype.configure = function () {
  var rcfile = '.raliorc';
  var config_path = path.join(process.env.HOME, rcfile);
  
  try {
    this.config = JSON.parse(fs.readFileSync(config_path));
  } catch (e){
    console.log('~/' + rcfile + ' doesn\'t exist!'.red)
    process.kill();
  }
